...
import { BoxMultipleFilled, BoxFilled, CheckmarkCircleFilled, AlertFilled, PeopleFilled } from "@fluentui/react-icons"
import { useEffect, useState } from "react"
import { motion } from "framer-motion"
import { useNavigate } from "react-router-dom"
import { getProducts, getStockCatalogues, getProductStatuses } from "../service/product"
...
export const Dashboard = () => {
    ...
    useEffect(() => {
        const fetchDashboardData = async () => {
            try {
                ...
                const [productsRes, cataloguesRes, statusesRes, usersRes] = await Promise.all([
                    getProducts(),
                    getStockCatalogues(),
                    getProductStatuses(),
    ...
    const getStatusColor = (statusName) => {
        ...
        if (name.includes('sellado') || name.includes('nuevo')) return 'text-success-600'
        if (name.includes('abierto') || name.includes('uso')) return 'text-warning-600'
        if (name.includes('vencido') || name.includes('caducado')) return 'text-danger-600'
    ...
    const userDisplayName = user?.name || user?.nombre || user?.fullName || user?.username || ""
        ...
        <div className="w-full min-h-full px-4 pb-6">
            <div className="flex flex-col gap-6 w-full max-w-[1400px] mx-auto">
                ...
                <div className="flex flex-col gap-2">
                    ...
                    <p className="text-sm text-background-500">Bienvenido de vuelta {userDisplayName}</p>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 w-full">
                    {statCards.map((stat, index) => {
                        const getNavigationPath = (title) => {
                            switch(title) {
                                case "Total Productos": return "/App/Products";
                                case "Catálogos": return "/App/StockCatalogues";
                                case "Estados": return "/App/ProductStatuses";
                                case "Usuarios": return "/App/Users";
                                default: return "/App";
                            }
                        };
                                                    ...
                                                    <CardBody className="p-4">
                                                        <div className="flex items-center justify-between gap-3">
                                                            ...
                                                            <div className="flex items-center gap-2 flex-shrink-0">
                                                                ...
                                                                <p className="text-xs text-background-500">productos</p>
                                                            </div>
                                                        </div>
                                                    </CardBody>
                                                </Card>
                                                        ...
                                                        <CardBody className="p-4">
                                                            <div className="flex items-center justify-between gap-3 w-full">
                                                                ...
                                                                <div className="flex items-center gap-2 flex-shrink-0">
                                                                    ...
                                                                    <p className="text-xs text-background-500">unidades</p>
                                                                </div>
                                                            </div>
                                                        </CardBody>
                                                    </Card>
