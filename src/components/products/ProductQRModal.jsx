                        ...
                        <div className="flex flex-col items-center gap-6 w-full overflow-x-hidden">
                            ...
                            <div className="w-full flex flex-col gap-2 bg-background-100 p-4 rounded-lg overflow-x-hidden">
                                <div className="flex justify-between gap-4">
                                    <p className="text-sm text-background-500 flex-shrink-0">Lote:</p>
                                    <p className="text-sm font-medium break-all text-right">{product?.lote}</p>
                                </div>
                                <div className="flex justify-between gap-4">
                                    <p className="text-sm text-background-500 flex-shrink-0">Lote Proveedor:</p>
                                    <p className="text-sm font-medium break-all text-right">{product?.loteProveedor}</p>
                                </div>
                                {product?.numeroSerie && (
                                    <div className="flex justify-between gap-4">
                                        <p className="text-sm text-background-500">N° Serie:</p>
                                        <p className="text-sm font-medium break-all text-right">{product.numeroSerie}</p>
                                    </div>
                                )}
                                {product?.stockCatalogueName && (
                                    <div className="flex justify-between gap-4">
                                        <p className="text-sm text-background-500">Catálogo:</p>
                                        <p className="text-sm font-medium break-all text-right">{product.stockCatalogueName}</p>
                                    </div>
                                )}
                                {product?.productStatusName && (
                                    <div className="flex justify-between gap-4">
                                        <p className="text-sm text-background-500">Estado:</p>
                                        <p className="text-sm font-medium break-all text-right">{product.productStatusName}</p>
                                    </div>
                                )}
                                {product?.unitOfMeasurementName && (
                                    <div className="flex justify-between gap-4">
                                        <p className="text-sm text-background-500">Unidad:</p>
                                        <p className="text-sm font-medium break-all text-right">{product.unitOfMeasurementName} ({product.unitOfMeasurementCode})</p>
                                    </div>
                                )}
                                {product?.warehouseTypeName && (
                                    <div className="flex justify-between gap-4">
                                        <p className="text-sm text-background-500">Tipo Almacén:</p>
                                        <p className="text-sm font-medium break-all text-right">{product.warehouseTypeName}</p>
                                    </div>
                                )}

                                {product?.codigoProducto && (
                                    <div className="flex justify-between gap-4">
                                        <p className="text-sm text-background-500">Código Producto:</p>
                                        <p className="text-sm font-medium break-all text-right">{product.codigoProducto}</p>
                                    </div>
                                )}

                                {product?.numeroAnalisis && (
                                    <div className="flex justify-between gap-4">
                                        <p className="text-sm text-background-500">N° Análisis:</p>
                                        <p className="text-sm font-medium break-all text-right">{product.numeroAnalisis}</p>
                                    </div>
                                )}

                                {product?.fecha && (
                                    <div className="flex justify-between gap-4">
                                        <p className="text-sm text-background-500">Fecha Ingreso:</p>
                                        <p className="text-sm font-medium break-all text-right">{product.fecha}</p>
                                    </div>
                                )}

                                {product?.caducidad && (
                                    <div className="flex justify-between gap-4">
                                        <p className="text-sm text-background-500">Fecha Caducidad:</p>
                                        <p className="text-sm font-medium break-all text-right">{product.caducidad}</p>
                                    </div>
                                )}

                                {product?.reanalisis && (
                                    <div className="flex justify-between gap-4">
                                        <p className="text-sm text-background-500">Fecha Reanálisis:</p>
                                        <p className="text-sm font-medium break-all text-right">{product.reanalisis}</p>
                                    </div>
                                )}
                                <div className="flex justify-between gap-4">
                                    <p className="text-sm text-background-500">Cantidad:</p>
                                    <p className="text-sm font-medium break-all text-right">{product?.cantidadSobrante}</p>
                                </div>
                                <div className="flex justify-between gap-4">
                                    <p className="text-sm text-background-500">Cantidad Total:</p>
                                    <p className="text-sm font-medium break-all text-right">{product?.cantidadTotal}</p>
                                </div>
                                <div className="flex justify-between gap-4">
                                    <p className="text-sm text-background-500">N° Contenedores:</p>
                                    <p className="text-sm font-medium break-all text-right">{product?.numeroContenedores}</p>
                                </div>
                                {product?.fabricante && (
                                    <div className="flex justify-between gap-4">
                                        <p className="text-sm text-background-500">Fabricante:</p>
                                        <p className="text-sm font-medium break-all text-right">{product.fabricante}</p>
                                    </div>
                                )}
                                {product?.distribuidor && (
                                    <div className="flex justify-between gap-4">
                                        <p className="text-sm text-background-500">Distribuidor:</p>
                                        <p className="text-sm font-medium break-all text-right">{product.distribuidor}</p>
                                    </div>
                                )}
                            </div>
