...
export const ProductsModal = ({isOpen, onOpenChange, data, initialData, action, onRefresh, closeDrawer}) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            addToast({
                title: success
                    ? `Se ${verb} el producto`
                    : `No se ${verb} el producto`,
                description: success ? "El producto está disponible en el sistema." : "Ocurrió un error al procesar la solicitud.",
                ...
                <CardBody className="pl-4">
                    ...
                    <div className="w-full flex flex-col gap-1">
                        <div className="w-full flex justify-between">
                            <p className="font-semibold break-all line-clamp-2 pr-4">Lote: {product.lote}</p>
                            {action !== "create" && (
                                <Tooltip
                        ...
                        <p className="text-sm line-clamp-1 break-all"><span className="font-medium">Lote Proveedor: </span>{product.loteProveedor}</p>
                        {product.numeroSerie && <p className="text-sm line-clamp-1 break-all"><span className="font-medium">N° Serie: </span>{product.numeroSerie}</p>}
                        ...
                        <p className="text-sm line-clamp-1 break-all"><span className="font-medium">Cantidad: </span>{product.cantidad}</p>
                        <p className="text-sm line-clamp-1 break-all"><span className="font-medium">Cantidad Total: </span>{product.cantidadTotal}</p>
                        <p className="text-sm line-clamp-1 break-all"><span className="font-medium">N° Contenedores: </span>{product.numeroContenedores}</p>
                    </div>
                </CardBody>
            </Card>
        );
                ...
                <ModalContent className="bg-background">
                    {(onClose) => (
                        ...
                        <ModalHeader {...moveProps} className="flex flex-col gap-2 pb-4 pt-4">
                            ...
                            <p className="text-lg font-bold text-center">¿Desea {action === "create" ? "registrar" : "actualizar"} el siguiente producto?</p>
                        </ModalHeader>
                        <ModalBody className="py-0 gap-0">
                            ...
                            {action === "create" ? 
                                productDetails(data)
                            :
                                !showBefore ? productDetails(data) : productDetails(initialData)
                            }
                        </ModalBody>
